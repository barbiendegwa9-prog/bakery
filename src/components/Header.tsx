        ...
        <div className="flex justify-between items-center h-20">
          ...
          >
            ...
            <div>
              ...
              <p className="text-[9px] sm:text-[10px] font-mono text-gray-400 tracking-wider uppercase">
                Artisan Sourdough & Custom Pastry
              </p>
            </div>
          </div>

          {/* Desktop Navigation with improved spacing and wrapping safety */}
          <nav className="hidden md:flex flex-wrap items-center gap-1.5 lg:gap-3 xl:gap-4 max-w-[55%] lg:max-w-[65%] justify-center">
            {navItems.map((item) => (
              <button
                key={item.id}
                id={`nav-${item.id}`}
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#c5a059] text-black text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center animate-bounce">
                  {cartCount}
                </span>
              )}
            </button>
